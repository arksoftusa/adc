...
- **Compiled Assets**: "For compiled or bundled software, all output MUST be directed to a git-ignored `dist/` directory. Within `dist/`, assets must be rigidly categorized into `dist/release/` (production artifacts), `dist/staging/` (pre-production environments), and `dist/build/` (intermediate cache/build files)."
