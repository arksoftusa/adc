...
- **Compiled Assets**: "For compiled or bundled software, all output MUST be directed to a git-ignored `dist/` directory at the root (or `src/dist/` for source-relative paths). For consistency, final artifacts are to be organized in `dist/release/`, pre-production artifacts in `dist/staging/`, and intermediate build cache files in `dist/build/`. If `src/dist/` exists for a compile pipeline, the same structure `src/dist/release/`, `src/dist/staging/`, `src/dist/build/` must also be used."
- **Utility Scripts**: "All supplementary bash, python, or Node.js scripts used for building, deploying, or local development MUST be placed in `src/scripts/` (or `scripts/` if a project has top-level scripts). No dangling scripts should exist at the project root."
...
- **Temporary Data**: "Any runtime logs, uploads, or temporary artifacts MUST be written to `logs/` or `tmp/` respectively. Both paths MUST be explicitly added to `.gitignore`."
